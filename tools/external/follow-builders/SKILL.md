...
version: 1.0.0
upstream: https://github.com/zarazhangrui/follow-builders @ 6df06d7 (2026-09-03)
description: Daily or weekly digest of what top AI builders are saying — remixes a centrally fetched feed of X posts, podcast transcripts and official blogs into summaries; delivers in-chat, via Telegram, or by email. Use when asked to "set up follow builders", "what are the AI builders saying", "my AI digest", "/follow-builders", or "/ai". Needs node and network; no API keys for reading. (kstack)
---

## When to invoke

Someone wants the AI-builders digest: first-time setup ("set up follow
builders"), an on-demand run ("/ai", "/follow-builders", "what are the builders
saying"), or a settings change ("switch to weekly", "change language to
Chinese"). Not a coding skill and not part of the stack router's decide / build /
review / land clusters.

Vendored third-party skill. Upstream is the `upstream:` URL in the frontmatter;
the body below is upstream's `SKILL.md` verbatim. Data is fetched from
upstream's `main` by `scripts/prepare-digest.js`, so the local copy needs no
feed files. Before first use run `npm ci` in `scripts/` (deps: `dotenv`,
`proper-lockfile`). `${CLAUDE_SKILL_DIR}` in the body resolves through the
`~/.claude/skills/follow-builders` symlink that `bin/install` creates.
