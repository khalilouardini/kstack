#!/usr/bin/env bash
# check-careful.sh — PreToolUse hook for the /careful skill
...
# pwd -P resolves to the PHYSICAL directory, so the helper is found the same way
# whether this script is invoked through the ~/.claude/skills/careful symlink or
...
# The [ -f ] existence check must come FIRST — an if-guard cannot catch the
# fatal case on its own.
_HOOK_HELPER="$SCRIPT_DIR/hook-extract.sh"
...
#
# A grep-based extractor
#   grep -o '"command"[[:space:]]*:[[:space:]]*"[^"]*"'
# stops at the first escaped quote in the JSON string value. Any destructive
# command preceded by a quoted argument is therefore truncated away before the
# pattern checks ever run:
#
...
# unreadable input.
set +e
CMD=$(kstack_hook_extract_field "$INPUT" command)
EXTRACT_RC=$?
set -e

# No parser available, or the payload is not parseable JSON. Fail closed.
if [ "$EXTRACT_RC" -ne 0 ] && [ -n "$INPUT" ]; then
  kstack_hook_decision ask "[careful] Could not parse the tool payload to safety-check this command. Approve only if you know what it does."
  exit 0
fi
...
if printf '%s' "$CMD" | grep -qE '\$\{IFS\}|\$IFS|\$\(echo[^)]*base64[^)]*\)|base64[[:space:]]+(-d|--decode)[^|]*\|[[:space:]]*(sh|bash)' 2>/dev/null; then
  kstack_hook_decision ask "[careful] Shell obfuscation detected (IFS word-splitting or base64-to-shell). Read the command carefully before approving."
  exit 0
fi
...
# installer flows, including this stack's own setup pattern.
_IS_SIMPLE=1
case "$CMD" in
  *';'*|*'&&'*|*'||'*|*'|'*|*$'\n'*) _IS_SIMPLE=0 ;;
esac
if [ "$_IS_SIMPLE" -eq 1 ]; then
  ...
  if printf '%s' "$CMD" | grep -qE '^[[:space:]]*(sudo[[:space:]]+)?rm[[:space:]]' 2>/dev/null \
    ...
    if [ "$_ROOT_TARGETS" -eq 1 ] && [ "$_SAFE_TARGETS" -eq 0 ]; then
      kstack_hook_decision deny "[careful][HIGH] Recursive delete of / or the home directory is blocked while /careful is active. If you truly mean it, end the /careful session first."
      exit 0
    fi
  fi
  ...
  if printf '%s' "$CMD" | grep -qE '^[[:space:]]*git[[:space:]]+push([[:space:]]|$)' 2>/dev/null; then
    ...
    if [ "$_HAS_FORCE" -eq 1 ]; then
      ...
      # Many worktrees and fresh clones lack the origin/HEAD symbolic ref —
      # without a fallback the HIGH tier would be silently inert exactly where
      # agents run. Probe the two conventional defaults.
      ...
      if [ -n "$_DEFAULT_BRANCH" ]; then
        ...
        if [ "$_TARGETS_DEFAULT" -eq 1 ]; then
          kstack_hook_decision deny "[careful][HIGH] Force-push to the default branch ($_DEFAULT_BRANCH) is blocked while /careful is active. Use --force-with-lease on a feature branch, or end the /careful session if you truly mean it."
          exit 0
        fi
      fi
    fi
...
# Three hardenings on top of the anchored shape:
...
#     the payload is a real newline (a grep extractor kept it as two literal
#     characters, which broke the anchored match by accident).
case "$CMD" in
...
# --- Additive operator patterns ---
# Config can only ADD warn rules, never remove or weaken a baseline family:
# this file is consulted AFTER the hardcoded checks and only when none of them
# matched, so no file content can suppress a baseline warning. One POSIX ERE
# per line; blank lines and #-comments skipped; an invalid regex is skipped
# (never fatal — the hook must not break on a typo in config).
if [ -z "$WARN" ]; then
  _PATTERN_FILE="${KSTACK_STATE:-$HOME/.kstack}/careful-patterns.txt"
  if [ -f "$_PATTERN_FILE" ]; then
    while IFS= read -r _PAT || [ -n "$_PAT" ]; do
      ...
      if printf '%s' "$CMD" | grep -qE -- "$_PAT" 2>/dev/null; then
        WARN="Operator rule matched: $_PAT"
        break
      fi
    done < "$_PATTERN_FILE"
  fi
...
# --- Output ---
if [ -n "$WARN" ]; then
  kstack_hook_decision ask "[careful] $WARN"
else
  echo '{}'
fi
